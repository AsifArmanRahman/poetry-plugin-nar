...
def test_build_package_dynamic_metadata(
    ...
    with ZipFile(nar, "r") as file:
        ...
        metadata = file.read(f"{name}/__about__.py").decode()

        assert f'__version__ = "{poetry.package.version}"' in metadata
        assert f'__description__ = "{poetry.package.description}"' in metadata
